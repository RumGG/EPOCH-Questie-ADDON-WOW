...
def fix_all_exclusiveto():
    ...
    for i, line in enumerate(lines):
        ...
        match = re.search(r',nil,nil,nil,nil,nil,(\d+),nil,', line)
        if match:
            original = line
            ...
            line = line.replace(f',nil,nil,nil,nil,nil,{zone_id},nil,', f',nil,nil,nil,nil,nil,nil,{zone_id},')
            ...
            if line != original:
                ...
                if quest_match:
                    print(f"Fixed quest {quest_match.group(1)}: moved zone {zone_id} from exclusiveTo to zoneOrSort")
